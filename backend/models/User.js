/**
 * User Model
 * Handles user authentication and profile data with Google OAuth support
 */
...
const userSchema = new mongoose.Schema({
  ...
  twilioPhoneNumber: {
    ...
    validate: {
      validator: function(v) {
        // Allow null or valid E.164 format (+1234567890)
        return v === null || /^\+[1-9]\d{1,14}$/.test(v);
      },
      message: 'Phone number must be in E.164 format (e.g., +15025211439)'
    }
  },
  // Multi-tenant Twilio credentials (single account per user)
  twilioCredentials: {
    accountSid: {
      type: String,
      default: null,
  ...
  // Multiple phone numbers from same Twilio account
  phoneNumbers: [{
    phoneNumber: {
      type: String,
      required: true,
    ...
    assignedAssistantId: {
      type: String, // Changed from ObjectId to String to match assistant.id format
      required: true,
      trim: true
    },
    twimlAppSid: {
