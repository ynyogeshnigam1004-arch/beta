...
class GroqService {
  constructor() {
    ...
    // Rate limiting - Balanced for free tier
    this.requestCounts = new Map(); // Track requests per minute
    this.lastResetTime = Date.now();
    this.maxRequestsPerMinute = 20; // Balanced limit (Groq free tier is 30, leaving 10 as buffer)
    this.requestQueue = [];
  ...
  validateAndCleanMessages(messages) {
    ...
    const cleanedMessages = messages.map((msg, index) => {
      ...
      // 1. Tool messages (role: 'tool') - must have tool_call_id and content
      if (msg.role === 'tool') {
        if (!msg.tool_call_id || !msg.content) {
          console.warn(`⚠️ Skipping invalid tool message ${index}`);
          return null;
        }
        return {
          role: msg.role,
      ...
      // 2. Assistant messages with tool_calls - content can be null/empty
      ...
      // 3. Regular messages - must have content
      if (!msg.content) {
        console.warn(`⚠️ Skipping message ${index}: missing content`);
        return null;
      }
  ...
  async convertWebMToWAV(webmBuffer) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        // Check minimum size for valid audio - skip small buffers silently
        ...
        if (!isValidWebM) {
          console.warn('⚠️ Invalid WebM format, attempting conversion anyway...');
        }
    ...
    try {
      ...
      if (format === 'webm') {
        ...
        try {
          ...
          // If buffer was too small, skip this chunk silently
          if (processedBuffer === null) {
            console.log('⏭️ Skipping small audio chunk');
            return { text: '' }; // Return empty transcription
          }
  ...
  async generateStreamingChatCompletion(messages, onChunk, options = {}) {
    try {
      ...
      if (options.tools && options.tools.length > 0) {
        ...
        console.log(`🔧 Added ${options.tools.length} tool(s) to request`);
      }
          ...
          try {
            ...
            for (const line of lines) {
              ...
              try {
                ...
                // Don't log warning for incomplete chunks
