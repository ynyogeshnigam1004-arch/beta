...
class CartesiaService {
  constructor() {
    ...
    // Available Cartesia models (updated for 2025 API)
    this.models = {
      'sonic-3': {
        id: 'sonic-3',
        name: 'Sonic 3.0',
        latency: 85,  // ~85ms (fastest)
        description: 'Latest Sonic 3 model - RECOMMENDED'
      },
      'sonic-2024-10': {
        id: 'sonic-2024-10',
        name: 'Sonic 2.0',
        latency: 95,  // ~95ms
        description: 'Sonic 2.0 model with improved quality'
      },
      'sonic-turbo': {
        id: 'sonic-turbo',
        name: 'Sonic Turbo',
    ...
    // Don't fetch voices on initialization - fetch on first use instead
    // This prevents startup errors if API key is invalid
    // Voices will be fetched automatically when first needed
  }
  ...
  async fetchVoicesFromAPI() {
    try {
      ...
      const voicesArray = response.data.data || response.data;
  ...
  async textToSpeech(text, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Debug logging
        console.log('🔍 Cartesia TTS Request Details:');
        console.log('   API URL:', `${this.apiUrl}/tts/bytes`);
        console.log('   API Key:', this.apiKey ? `${this.apiKey.substring(0, 15)}...${this.apiKey.substring(this.apiKey.length - 4)}` : 'NOT SET');
        console.log('   Model:', model, '→', (this.models[model]?.id) || 'sonic-english');
        ...
        console.error(`❌ Cartesia TTS error (attempt ${attempt}/${maxRetries}):`);
        ...
        if (isUnauthorized) {
          console.error('\n🔑 401 UNAUTHORIZED ERROR - DETAILED DIAGNOSIS:');
          console.error('   ═══════════════════════════════════════════════');
          console.error('   API Key being used:', this.apiKey ? `${this.apiKey.substring(0, 15)}...${this.apiKey.substring(this.apiKey.length - 4)}` : 'NOT SET');
          console.error('   API Key length:', this.apiKey ? this.apiKey.length : 0);
          console.error('   API Key starts with:', this.apiKey ? this.apiKey.substring(0, 7) : 'N/A');
          console.error('   Expected format: sk_car_...');
          console.error('   API Endpoint:', `${this.apiUrl}/tts/bytes`);
          console.error('   Header Used: X-API-Key');
          console.error('   ═══════════════════════════════════════════════');
          console.error('   Possible Causes:');
          console.error('   1. ❌ API key is invalid or expired');
          console.error('   2. ❌ API key has IP restrictions');
          console.error('   3. ❌ API key usage limits exceeded');
          console.error('   4. ❌ API key format is incorrect');
          console.error('   5. ❌ API key not activated on Cartesia dashboard');
          console.error('   ═══════════════════════════════════════════════');
          console.error('   Solution: Get a new API key from https://cartesia.ai/');
          console.error('   ═══════════════════════════════════════════════\n');
          throw new Error(`Cartesia API key is invalid (401 Unauthorized). Please update CARTESIA_API_KEY in .env file.`);
        }
    ...
    try {
      ...
      if (shouldConvert) {
        ...
        if (textToConvert.length > 0) {
          ...
          // IMPORTANT: Clear buffer after conversion to prevent duplicate audio
          buffer.text = '';
        }
      }
  ...
  async flushBuffer(buffer, onAudioChunk, options = {}) {
    try {
      ...
      // Only flush if there's actually text remaining
      if (remainingText.length > 0) {
        console.log('🔄 Flushing remaining buffer:', remainingText);
        await this.textToSpeechStreaming(remainingText, onAudioChunk, options);
        buffer.text = '';
      } else {
        console.log('✅ Buffer already empty, skipping flush');
      }
    } catch (error) {
