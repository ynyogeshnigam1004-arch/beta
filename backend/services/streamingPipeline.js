...
class StreamingPipeline extends EventEmitter {
  constructor() {
    ...
    this.elevenLabsService = require('./elevenLabsService'); // Import as instance
    ...
    // Voice configuration (will be set from assistant config)
    this.voiceProvider = 'cartesia';
    this.voiceModel = 'sonic-2024-10';
    this.voiceId = '95d51f79-c397-46f9-b49a-23763d3eaa2d'; // Arushi voice (HARDCODED FIX)
  ...
  getTTSService(provider = null) {
    ...
    switch (ttsProvider.toLowerCase()) {
      ...
      default:
        console.warn(`⚠️ Unknown TTS provider: ${ttsProvider}, using default: elevenlabs`);
        return this.elevenLabsService;
    }
  }
  ...
  getFallbackTTSService(currentTtsService) {
    // Always use ElevenLabs as fallback since Cartesia has API key issues
    if (currentTtsService === this.cartesiaService) {
      return this.elevenLabsService;
    } else if (currentTtsService === this.elevenLabsService) {
      return null; // No fallback from ElevenLabs
    }
    return this.elevenLabsService; // Default to ElevenLabs
  }
