...
class APIDataManager {
  ...
  validateAPIKeys() {
    ...
    if (!this.cartesiaApiKey || this.cartesiaApiKey === 'your_cartesia_api_key_here') {
      missingKeys.push('CARTESIA_API_KEY');
    }
  ...
  async fetchAllCartesiaData() {
    ...
    try {
      ...
      if (voicesResponse.data && Array.isArray(voicesResponse.data)) {
        voicesResponse.data.forEach(voice => {
          voices[voice.id] = {
            id: voice.id,
            name: voice.name,
      ...
      console.error('❌ CRITICAL ERROR: Failed to fetch Cartesia data:', error.message);
      throw new Error(`Failed to fetch Cartesia data: ${error.message}`);
    }
  }
  ...
  async fetchAndSaveAllData() {
    console.log('🚀 STARTING MANDATORY API DATA FETCH...');
    console.log('⚠️  NO FALLBACKS - API DATA IS REQUIRED');
    
    try {
      // Fetch all data in parallel
      const [groqData, cartesiaData] = await Promise.all([
        this.fetchAllGroqModels(),
        this.fetchAllCartesiaData()
      ]);
